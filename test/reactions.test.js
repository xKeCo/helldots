...
describe("the reaction bar", () => {
  ...
  it("gives no pill a hover tooltip, only an accessible name", () => {
    // The emoji and count already say what the pill is, and a bubble on every
    // pill turned a dense row into a wall of popups. Only the trigger beside
    // the row keeps a tooltip. The accessible name still carries the action.
    const [mine, theirs] = pillsOf(
      barFor({ "\u{1F44D}": ["me"], "\u{1F680}": ["ana"] })
    );
    expect(mine.dataset.hdTooltip).toBeUndefined();
    expect(theirs.dataset.hdTooltip).toBeUndefined();
    expect(mine.getAttribute("aria-label")).toContain(en.reactionToggleOff);
    expect(theirs.getAttribute("aria-label")).toContain(en.reactionToggleOn);
  });
